...
const fakeData = {
    add: [
        {
            movie: 'Avenger: The Infinity war',
            date: '26/06/2025',
            point: 20,
        },
        {
...
const ScoreHistory = () => {
    ...
    const handleView = () => {
        ...
        const filteredAdd = fakeData.add.filter((item) =>
            isInRange(item.date)
        );
        const filteredUse = fakeData.use.filter((item) =>
            isInRange(item.date)
        );
    ...
    return (
        <UserDashboardLayout>
            ...
            <div className="bg-[#0d1117] border border-gray-700 rounded-md mb-8">
                <div className="border-b border-gray-700 p-4 font-semibold text-white flex items-center gap-2">
                    ...
                    <ul className="divide-y divide-gray-800">
                        {result.add.map((item, idx) => (
                            <li key={idx} className="px-4 py-3 text-white flex justify-between items-center">
                                <span>
                                    {item.movie} <span className="text-gray-400 text-sm">(Movie Purchased)</span>
                                </span>
                ...
                <div className="bg-[#0d1117] border border-gray-700 rounded-md mb-8">
                    ...
                    <ul className="divide-y divide-gray-800">
                        {result.use.map((item, idx) => (
                            <li key={idx} className="px-4 py-3 text-white flex justify-between items-center">
                                <span>
                                    {item.movie} <span className="text-gray-400 text-sm">(Ticket Redemption)</span>
                                </span>
