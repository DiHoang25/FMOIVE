...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const nowShowingInterval = setInterval(() => {
      setNowShowingIndex((prev) => Math.min(prev + 1, Math.max(0, nowShowingMovies.length - itemsPerPage)));
    }, 3000);

    const comingSoonInterval = setInterval(() => {
      setComingSoonIndex((prev) => Math.min(prev + 1, Math.max(0, comingSoonMovies.length - itemsPerPage)));
    }, 3000);
  ...
  return (
    <div className="bg-black text-white px-6 py-10 space-y-10">
      ...
      <div className="mt-10"> <h2 className="text-xl font-bold mb-4"> Hot movie : <span className="text-red-500">May 2025</span> </h2>
        {hotMovies.slice(0, 4).map((movie, idx) => (
          <div key={idx} className="flex items-start gap-4 mb-6 border-b border-gray-700 pb-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 text-sm font-medium mb-1">
                ...
                <span className="bg-white text-black px-1 rounded text-m">{movie.version}</span>
              ...
              <p className="text-xl text-gray-300 mb-2">
                Now showing • Ends: {formatDate(movie.end_date)}
              </p>

              <p className="text-2xl mb-2">
                Actor: {movie.actors || 'N/A'}
              </p>
              <p className="text-2xl mb-2">
                Description: {movie.description || 'N/A'}
              </p>
              <div className="mb-2">
                <button onClick={() => openTrailer(movie.trailer_link)} className="bg-gray-800 text-white px-5 py-2 rounded text-2m ml-0">
                  Watch trailer
                </button>
              </div>
