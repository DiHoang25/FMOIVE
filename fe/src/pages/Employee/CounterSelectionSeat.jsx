...
// Removed imports: TransformWrapper, TransformComponent, useMediaQuery, Monitor, Crown, message
...
const SeatSelection = () => {
    ...
    const [selectedSeatsState, setSelectedSeatsState] = useState(selectedSeats || []);
    ...
    const toggleSeat = (seat) => {
        setSelectedSeatsState((prev) => {
            ...
            const selectedSeatObjects = roomData?.seats?.filter(s => updated.includes(s.label)) || [];
    ...
    const getSeatClass = (seat) => {
        ...
        const base = "w-10 h-10 text-xs rounded-lg flex items-center justify-center font-bold transition-all duration-200 transform hover:scale-110 hover:shadow-lg border-2";

        if (seat.status === "booked") return `${base} bg-gray-600 text-white cursor-not-allowed opacity-70`;
                ...
                <div className="max-w-6xl mx-auto px-6 py-6 bg-slate-800 rounded-2xl shadow-lg">
                    ...
                    {roomData ? (
                        ...
                        <div className="overflow-x-auto text-center">
                            <div className="inline-block space-y-3 min-w-[320px] mx-auto"> {/* min-w to ensure content width, inline-block to allow centering with mx-auto */}
                                {[...Array(roomData.rows)].map((_, rowIndex) => {
                                    const rowLetter = String.fromCharCode(65 + rowIndex);
                                    const rowSeats = roomData.seats.filter((s) => Number(s.row) === rowIndex + 1);

                                    return (
                                        <div key={rowLetter} className="flex items-center justify-center gap-2"> {/* Adjusted gap for consistent look */}
                                            <div className="w-8 flex items-center justify-center">
                                                <span className="text-slate-400 font-bold text-sm">{rowLetter}</span>
                                            </div>
                                            <div className="flex gap-2 justify-center"> {/* Adjusted gap for consistent look */}
                                                {[...Array(roomData.columns)].map((_, colIndex) => {
                                                    const seat = rowSeats.find((s) => Number(s.column) === colIndex + 1);
                                                    if (!seat) return <div key={`empty-${colIndex}`} className="w-10 h-10" />; // Keep consistent size
                    ...
                    <div className="flex justify-around text-sm mt-6 flex-wrap gap-y-2">
                        ...
                        <div className="flex items-center gap-1">
                            <div className="w-4 h-4 bg-gradient-to-br from-amber-400 to-yellow-500 rounded-lg border-2 border-amber-300 flex items-center justify-center">
                                ...
                                <span className="text-gray-900 font-bold text-[8px]">VIP</span>
                            </div>
