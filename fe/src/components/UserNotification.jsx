...
const UserNotification = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const newComing = (data?.comingSoon || []).sort((a, b) => new Date(a.start_date) - new Date(b.start_date))
            const newHot = data?.hotMovies || []
            if (newComing.length > comingSoon.length || newHot.length > hotMovies.length) {
                setHasNew(true)
            }
            setComingSoon(newComing)
            setHotMovies(newHot)
        } catch (err) {
            console.error(err)
        ...
        <>
            ...
            >   
                {error ? (
                    ...
                    />
                ) : comingSoon.length + hotMovies.length === 0 ? (
                    <div className="text-center py-6 text-gray-400 text-sm">🎬 No movie updates available.</div>
                ) : (
